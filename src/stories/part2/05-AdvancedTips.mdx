...
│  │ M src/api.ts       │ a1b2c3 feat: tag    │ │
│  │ A src/types/tag.ts │ d4e5f6 fix: auth    │ │
│  │ M tests/api.test   │ g7h8i9 refactor     │ │
│  ├────────────────────┼─────────────────────┤ │
│  │ Modified Files     │ System Resources    │ │
│  │ src/api.ts         │ CPU: ████░░░░ 48%   │ │
│  │ src/types/tag.ts   │ MEM: ██████░░ 72%   │ │
│  │ tests/api.test.ts  │ PID    CPU%  MEM%   │ │
│  ├────────────────────┼─────────────────────┤ │
│  │ Disk & Memory      │ File Counts         │ │
│  │ /dev/sda1 45G/100G │ JS: 32              │ │
│  │ Mem: 8.2G/16G      │ TS: 18              │ │
│  └────────────────────┴─────────────────────┘ │
│                                              │
└──────────────────────────────────────────────┘
...
에이전트에 문제 발생
    │
    ├─ 에이전트가 응답하지 않음?
    │   ├─ 1-2분 기다려봄 → 재개되면 정상 (API 대기)
    │   ├─ 계속 멈춤 → Ctrl-C로 중단 후 재시도
    │   └─ "Rate limit" 메시지 → 에이전트 수 줄이고 대기
    │
    ├─ 파일 충돌 발생?
    │   ├─ git status로 충돌 파일 확인
    │   ├─ 충돌 수동 해결 (에디터에서)
    │   └─ Team Lead에게 수정 사실 알리고 재시도
    │
    ├─ 시스템 리소스 부족?
    │   ├─ htop으로 CPU/메모리 확인
    │   ├─ 불필요한 에이전트 종료
    ...
    ├─ 에이전트가 잘못된 코드를 작성?
    │   ├─ git diff로 변경사항 확인
    │   ├─ git checkout -- [파일]로 변경 취소
    │   └─ 더 구체적인 프롬프트로 재시도
    │
    └─ tmux 세션이 사라짐?
        ├─ tmux ls로 세션 목록 확인
        ├─ 세션 있으면 → tmux attach -t [이름]
        └─ 세션 없으면 → 스크립트로 재구성
                         (pipe-pane 로그로 진행 상황 확인)
```
